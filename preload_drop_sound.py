#!/usr/bin/env python3
"""
Preload drop hit sound to eliminate playback delay
"""
...
# 1. Add a ref for the drop hit audio after other audio refs
old_refs = """  const coinImageRef = useRef<HTMLImageElement[] | null>(null)
  const lastFrameTimeRef = useRef(0)"""

new_refs = """  const coinImageRef = useRef<HTMLImageElement[] | null>(null)
  const lastFrameTimeRef = useRef(0)
  const dropHitAudioRef = useRef<HTMLAudioElement | null>(null)"""

content = content.replace(old_refs, new_refs)

# 2. Add useEffect to preload the audio
old_coin_loading = """  // Load coin images (4 frames)
  useEffect(() => {
...
new_coin_loading = """  // Preload drop hit sound for instant playback
  ...
  // Load coin images (4 frames)
  useEffect(() => {
    const coin1 = new Image()"""

content = content.replace(old_coin_loading, new_coin_loading)

# 3. Update playOuchSound to use preloaded audio
